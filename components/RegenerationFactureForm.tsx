...
export default function RegenerationFactureForm() {
  ...
  return (
    <div className="bg-white rounded-xl p-8 md:p-12 flex flex-col gap-8 text-black max-w-2xl">
      <h2 className="text-2xl font-bold text-[#0ea5e9]">{t('Régénération de facture')}</h2>

      {step === 'email' && (
        <>
          <p className="text-sm text-gray-600">
            {t("Entrez votre email d'inscription — nous vous enverrons un code de vérification.")}
          </p>
          ...
          <button onClick={sendCode} disabled={loading}
            ...
            {loading ? t('Envoi en cours...') : t('Envoyer le code')}
          </button>
        </>
      )}
      ...
      {step === 'code' && (
        <>
          <p className="text-sm text-gray-600">{t('Un code à 4 chiffres a été envoyé à')} <b>{email}</b>.</p>
          <div className="flex flex-col gap-1.5 max-w-md">
            <label className="text-sm font-bold">{t('Code')} *</label>
      ...
      {step === 'done' && (
        <div className="flex flex-col gap-4">
          <p className="text-sm text-green-600 font-bold">✓ {t('Identité vérifiée.')} {records.length} {t('inscription(s) trouvée(s).')}</p>
          {records.map((r) => (
            <div key={r.id} className="border rounded-lg p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <p className="text-sm">{t('Statut:')} <b>{r.STATUS}</b></p>
              </div>
